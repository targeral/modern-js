...
// EXTERNAL MODULE: ../../packages/cli/doc-core/src/theme-default/index.ts + 32 modules
var theme_default = __webpack_require__(92494);
...
/* harmony export */ });
/* unused harmony exports QUERY_REGEXP, HASH_REGEXP, MDX_REGEXP, externalLinkRE, addLeadingSlash, removeLeadingSlash, removeTrailingSlash, parseUrl */
...
const matchCache=new WeakMap();const match=item=>{if(matchCache.has(item)){return matchCache.get(item);}if(item.link&&activeMatcher(item.link)){matchCache.set(item,true);return true;}if('items'in item){const result=item.items.some(child=>match(child));if(result){matchCache.set(item,true);return true;}}matchCache.set(item,false);return false;};const traverse=item=>{if('items'in item){item.items.forEach(traverse);if(match(item)){item.collapsed=false;}}};sidebarData.forEach(traverse);setSidebarData(rawSidebarData.filter(Boolean).flat());},[props.sidebarData]);const removeLangPrefix=path=>{return path.replace(langRoutePrefix,'');};const activeMatcher=path=>isActive((0,runtime_utils/* removeBase */.E9)(removeLangPrefix(pathname)),removeLangPrefix(path),true);const preloadLink=link=>{const match=(0,router/* matchRoutes */.fp)(virtual_routes/* routes */._,link);if(match!==null&&match!==void 0&&match.length){const{route}=match[0];route.preload();}};return/*#__PURE__*/(0,jsx_runtime.jsx)("aside",{className:`modern-scrollbar ${Siderbar_index_module.sidebar} ${isSidebarOpen?Siderbar_index_module.open:''}`,children:/*#__PURE__*/(0,jsx_runtime.jsx)("div",{m:"t-1",children:/*#__PURE__*/(0,jsx_runtime.jsx)("nav",{children:sidebarData.map((item,index)=>{var _collapsed;return/*#__PURE__*/(0,jsx_runtime.jsx)(SidebarItemComp,{id:String(index),item:item,depth:0,activeMatcher:activeMatcher// The siderbarData is stable, so it's safe to use index as key
