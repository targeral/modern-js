...
import { IAppContext, getAllContextAPI, initAppContext } from './context';
...
export type CoreOptions<UserConfig> = {
  version?: string;
  configFile?: string;
  serverConfigFile?: string;
  packageJsonConfig?: string;
  plugins?: typeof INTERNAL_PLUGINS;
  transformPlugin?: TransformPlugin<UserConfig>;
...
const createCli = () => {
  ...
  const init = async <UserConfig>(
    ...
    const loaded = await loadUserConfig<UserConfig>(
      appDirectory,
    ...
    const { ConfigContext, AppContext, ResolvedConfigContext, useAppContext } =
      getAllContextAPI<UserConfig | DefaultUserConfig>();
  ...
  async function run<UserConfig>(argv: string[], options?: CoreOptions) {
    const { loadedConfig, appContext, resolved } = await init<UserConfig>(
      argv,
      options,
    );
  ...
  async function restart() {
    const { AppContext } = getAllContextAPI();
    isRestart = true;
